...
import { getSeperatorsPositions, getDefaultCountry } from '../utils/methods'
...
export const PhoneInput: React.FC<NumberFormatterProps> = (props: NumberFormatterProps) => {
  ...
  useEffect(() => {
    ...
    inputElm.current?.addEventListener('focus', () => {
      if (parent.current) {
        document
        ...
        document.getElementsByClassName('react-number-formatter-dropdown-content')[0].classList.remove('show')
      }
    })
    inputElm.current?.addEventListener('focusout', () => {
      if (parent.current) {
        document
          .getElementsByClassName('react-number-formatter-form')[0]
    ...
    if (props.includeDialingCode === undefined || props.includeDialingCode) {
      for (let i = 0; i < c.length; i++) {
        ...
        if (text.startsWith(pre)) {
          text = value.toString().slice(c[i].d.length - 1, text.length)
  ...
  return (
    <div className='react-number-formatter-form' ref={(ref: HTMLDivElement) => (parent.current = ref)}>
      <label>
        {!props.format && (
          <CountrySelector
      ...
      <input
        id='react-number-formatter-by-faraz'
